const gameName = "Visual memory";
const cart = document.createElement('div');
cart.classList.add('cart_memory');
gameScore = 1; // Initialiser avec une carte gagnante
...
function handleCardClick(event) {
    ...
    if (card.dataset.winning) {
        ...
        if (correctClicks === gameScore) {
            
            if (!(gameScore < document.querySelectorAll('.cart_memory').length/ 2)) {
                numberOfCards+= gridSize*2+1
            }
            score.innerHTML = "Actual score :  " + gameScore;
            gameScore++;
            setTimeout(() => {
